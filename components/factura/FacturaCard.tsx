import React from 'react';
import { useRouter } from 'next/router';
import { 
  Eye, Edit2, Trash2, Download, CheckCircle, XCircle, Clock, 
  AlertTriangle, Euro, Calendar, Building2, Truck 
} from 'lucide-react';

interface Factura {
  _id: string;
  numarFactura: string;
  cursaLegata: {
    _id: string;
    idCursa: string;
    pornire: string;
    destinatie: string;
    costNegociat: number;
    status: string;
  };
  partenerAsignat: {
    _id: string;
    numeFirma: string;
    contactPersoana: string;
    telefon: string;
    email: string;
  };
  suma: number;
  moneda: string;
  dataEmisa: string;
  scadenta: string;
  status: string;
  dataPlata?: string;
  note?: string;
  esteIntarziata: boolean;
  zilePanaLaScadenta: number | null;
  sumaFormatata: string;
}

interface FacturaCardProps {
  factura: Factura;
  userRole?: string;
  onEdit: (facturaId: string) => void;
  onDelete: (facturaId: string) => void;
  onStatusChange: (facturaId: string, newStatus: string) => void;
  onExport: (factura: Factura) => void;
  formatDate: (date: string) => string;
  getStatusBadge: (status: string) => { icon: JSX.Element; className: string; label: string };
}

export default function FacturaCard({
  factura,
  userRole,
  onEdit,
  onDelete,
  onStatusChange,
  onExport,
  formatDate,
  getStatusBadge
}: FacturaCardProps) {
  const router = useRouter();
  const statusInfo = getStatusBadge(factura.status);

  return (
    <div className="bg-white shadow-lg rounded-xl border border-slate-200 p-6">
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1">
          <h3 className="font-semibold text-slate-900 text-base">
            #{factura.numarFactura}
          </h3>
          <p className="text-sm text-slate-600">
            {factura.partenerAsignat.numeFirma}
          </p>
          
          <span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full mt-2 ${statusInfo.className}`}>
            {statusInfo.icon}
            {statusInfo.label}
          </span>
          
          {factura.esteIntarziata && factura.status !== 'platita' && (
            <span className="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-error-100 text-error-800 ml-2">
              <AlertTriangle className="w-3 h-3 mr-1" />
              Întârziată
            </span>
          )}
        </div>

        <div className="flex space-x-1">
          <button
            onClick={() => router.push(`/facturi/${factura._id}`)}
            className="p-2 text-slate-400 hover:text-blue-600 touch-target"
            title="Vezi detalii"
          >
            <Eye className="w-4 h-4" />
          </button>

          <button
            onClick={() => onExport(factura)}
            className="p-2 text-slate-400 hover:text-emerald-600 touch-target"
            title="Export PDF"
          >
            <Download className="w-4 h-4" />
          </button>
          
          <button
            onClick={() => onEdit(factura._id)}
            className="p-2 text-slate-400 hover:text-blue-600 touch-target"
            title="Editează"
          >
            <Edit2 className="w-4 h-4" />
          </button>
          
          {userRole === 'admin' && (
            <button
              onClick={() => onDelete(factura._id)}
              className="p-2 text-slate-400 hover:text-error-600 touch-target"
              title="Șterge"
            >
              <Trash2 className="w-4 h-4" />
            </button>
          )}
        </div>
      </div>

      <div className="space-y-2 text-sm">
        <div className="flex items-center text-slate-600">
          <Truck className="w-4 h-4 mr-2 flex-shrink-0" />
          <span className="truncate">
            {factura.cursaLegata.idCursa} ({factura.cursaLegata.pornire} → {factura.cursaLegata.destinatie})
          </span>
        </div>
        
        <div className="flex items-center text-slate-600">
          <Calendar className="w-4 h-4 mr-2 flex-shrink-0" />
          <span>Emisă: {formatDate(factura.dataEmisa)}</span>
        </div>
        
        <div className="flex items-center text-slate-600">
          <Calendar className="w-4 h-4 mr-2 flex-shrink-0" />
          <span>Scadență: {formatDate(factura.scadenta)}</span>
          {factura.zilePanaLaScadenta !== null && factura.status !== 'platita' && (
            <span className={`ml-2 text-xs ${factura.zilePanaLaScadenta < 0 ? 'text-error-600' : 'text-warning-600'}`}>
              ({factura.zilePanaLaScadenta > 0 ? `${factura.zilePanaLaScadenta} zile` : `${Math.abs(factura.zilePanaLaScadenta)} zile întârziere`})
            </span>
          )}
        </div>
        
        <div className="flex items-center justify-between pt-2 border-t border-slate-200">
          <div className="flex items-center">
            <Euro className="w-4 h-4 mr-2 text-primary-600" />
            <span className="font-semibold text-slate-900">
              {factura.sumaFormatata}
            </span>
          </div>
          
          {factura.status === 'Emisă' && (
            <button
              onClick={() => onStatusChange(factura._id, 'Trimisă')}
              className="text-xs btn btn-secondary px-2 py-1"
            >
              Marchează trimisă
            </button>
          )}
          
          {factura.status === 'Trimisă' && (
            <button
              onClick={() => onStatusChange(factura._id, 'Plătită')}
              className="text-xs btn btn-primary px-2 py-1"
            >
              Marchează plătită
            </button>
          )}
        </div>
      </div>
    </div>
  );
}